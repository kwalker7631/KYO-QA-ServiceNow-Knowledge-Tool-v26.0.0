...
def setup_high_contrast_styles(app):
    ...
    style.configure('.', background=KyoceraColors.BACKGROUND_MAIN, foreground=KyoceraColors.TEXT_DARK, font=('Segoe UI', 11))
    ...
    style.configure('Card.TFrame', background=KyoceraColors.WIDGET_BG, relief='solid', borderwidth=1, bordercolor=KyoceraColors.BORDER_COLOR)
    style.configure('Card.TLabel', background=KyoceraColors.WIDGET_BG)
    style.configure('Card.TLabelframe', background=KyoceraColors.WIDGET_BG, relief='solid', borderwidth=1, bordercolor=KyoceraColors.BORDER_COLOR)
    style.configure('Card.TLabelframe.Label', background=KyoceraColors.WIDGET_BG, foreground=KyoceraColors.TEXT_DARK, font=('Segoe UI', 12, 'bold'))
    style.configure('TButton', font=('Segoe UI', 11), padding=10, relief='raised', borderwidth=1, bordercolor=KyoceraColors.BORDER_COLOR)
    style.map('TButton', background=[('active', KyoceraColors.PRIMARY_ACTION), ('!disabled', KyoceraColors.WIDGET_BG)], foreground=[('active', 'white')])
    ...
    style.configure('Footer.TButton', font=('Segoe UI', 9), padding=(5, 3), relief='flat')
    style.map('Footer.TButton', background=[('active', '#e2e6ea')], bordercolor=[('!active', KyoceraColors.BACKGROUND_MAIN)])
    ...
    style.configure("Success.TLabel", foreground=KyoceraColors.STATUS_SUCCESS, background=KyoceraColors.WIDGET_BG, font=('Segoe UI', 14, 'bold'))
    style.configure("Review.TLabel", foreground=KyoceraColors.STATUS_WARNING, background=KyoceraColors.WIDGET_BG, font=('Segoe UI', 14, 'bold'))
    style.configure("Error.TLabel", foreground=KyoceraColors.STATUS_ERROR, background=KyoceraColors.WIDGET_BG, font=('Segoe UI', 14, 'bold'))
    style.configure("Default.TLabel", foreground=KyoceraColors.TEXT_DARK, background=KyoceraColors.WIDGET_BG, font=('Segoe UI', 14))
    style.configure("OCR.TLabel", foreground=KyoceraColors.PRIMARY_ACTION, background=KyoceraColors.WIDGET_BG, font=('Segoe UI', 14, 'bold'))
...
def create_live_status_section(parent, app):
    ...
    ttk.Label(activity_frame, text="Activity:", font=('Segoe UI', 11, 'bold'), style='Card.TLabel').grid(row=0, column=0, padx=(0, 10))
    ...
    ttk.Label(activity_frame, textvariable=app.progress_percent_var, style='Card.TLabel', font=('Segoe UI', 10, 'bold')).grid(row=1, column=3, padx=(10, 0), sticky='w')
    ...
    stats = [("Processed", app.count_processed, "Default.TLabel"), ("Passed", app.count_pass, "Success.TLabel"), 
             ("Needs Review", app.count_review, "Review.TLabel"), ("Errors", app.count_fail, "Error.TLabel"),
             ("OCR Used", app.count_ocr, "OCR.TLabel")] # ADDED OCR
    for i, (text, var, style) in enumerate(stats):
        f = ttk.Frame(stats_frame, style='Card.TFrame')
        f.grid(row=0, column=i, sticky='ew', padx=5)
        ttk.Label(f, text=text, font=('Segoe UI', 10, 'bold'), style='Card.TLabel').pack()
    ...
    review_pane.rowconfigure(1, weight=1); review_pane.columnconfigure(0, weight=1)
    bottom_pane.add(review_pane, weight=1)
    ttk.Label(review_pane, text="Files for Review", font=('Segoe UI', 12, 'bold'), style='Card.TLabel').grid(row=0, column=0, sticky='w', pady=(0,10))
    app.review_tree = ttk.Treeview(review_pane, columns=('file',), show='headings', height=4)
    app.review_tree.grid(row=1, column=0, sticky='nsew')
    app.review_tree.heading('file', text='File Name')
    review_scroll = ttk.Scrollbar(review_pane, orient="vertical", command=app.review_tree.yview)
    review_scroll.grid(row=1, column=1, sticky='ns')
    app.review_tree.configure(yscrollcommand=review_scroll.set)
    app.review_file_btn = ttk.Button(review_pane, text="🔍 Review Selected File", command=app.open_review_for_selected_file, state=tk.DISABLED)
    ...
    log_pane.rowconfigure(1, weight=1); log_pane.columnconfigure(0, weight=1)
    bottom_pane.add(log_pane, weight=2)
    ttk.Label(log_pane, text="Processing Log", font=('Segoe UI', 12, 'bold'), style='Card.TLabel').grid(row=0, column=0, sticky='w', pady=(0,10))
    app.log_text = tk.Text(log_pane, wrap=tk.WORD, state=tk.DISABLED, relief=tk.FLAT, font=("Consolas", 10), bg=KyoceraColors.WIDGET_BG, fg=KyoceraColors.TEXT_DARK, borderwidth=0, highlightthickness=0)
...
def create_footer(parent, app):
    ...
    app.fullscreen_status_label = ttk.Label(footer_frame, textvariable=app.fullscreen_status_var, style='Card.TLabel')
    app.fullscreen_status_label.pack(side='left')
    ttk.Button(footer_frame, text="Exit Application", command=app.on_closing, style="Footer.TButton").pack(side='right')
