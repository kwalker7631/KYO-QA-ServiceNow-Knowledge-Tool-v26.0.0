...
def parse_data(
    ...
    This version correctly checks the nested 'meta' field for models.
    """
    ...
    for item in raw_data:
        ...
        # Check if models were found inside the 'meta' dictionary
        elif not item.get("meta", {}).get("models"):
            item["status"] = "Needs Review"
            item["review_reason"] = "No device models were found."
            review_items.append(item)
            app_state.add_review_item(item)
        # If models were found, it's a pass
        else:
            item["status"] = "Pass"
