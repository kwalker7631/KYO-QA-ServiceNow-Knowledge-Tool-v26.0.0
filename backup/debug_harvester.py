...
from utils import MODEL_PATTERNS
import time
...
def test_model_extraction(pdf_path: Path):
    ...
    print(
        f"✅ Text extracted in {end_time - start_time:.2f} seconds ({len(raw_text)} characters)."
    )
    ...
    if found_models_str and found_models_str != "Not Found":
        ...
        print(
            "-> This means the patterns in 'config.py' did not match any text in the PDF."
        )
    print("=" * 60)
